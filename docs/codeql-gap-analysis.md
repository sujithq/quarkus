...
- Generated model file applied: no (format error)
...
- Finding location: DoctypeShareFolderMapping.java:35 (EntityManager.createNativeQuery)
...
- Finding locations:
  - DoctypeShareFolderMapping.java:35 (EntityManager.createNativeQuery)
  - DoctypeShareFolderMapping.java:55 (PanacheEntityBase.list)

### Generated Model Pack

- Result count: N/A (model load failed)
- Expected: JPA control finding plus Panache finding
- Panache finding present: N/A
- Generated sink exercised: N/A

### Format Error Details

The generated model file `.codeql/models/generated-sql-injection-sinks.yaml` uses an incomplete format:
...
CodeQL error:
```
Found '"io.quarkus....", "list", "Argument[0]", "sql-injection"', which does not match the signature 
'sinkModel(string package, string type, boolean subtypes, string name, string signature, string ext, 
string input, string kind, string provenance, [int origin])'
```

The generated model is missing:
1. Package field (should be separate from type)
2. Subtypes field (boolean)
3. Method signature field (empty string for any signature)
4. Extension field (empty string)
5. Provenance field (e.g., "manual")

### Validation Confidence

- low

The generated model file structure matches the Panache sinks identified in the gap analysis, but the format is incompatible with CodeQL's current extensible model schema. Executable validation could not be completed due to this format error.

status: VERIFICATION_BLOCKED
next: FIX_GENERATED_MODEL
