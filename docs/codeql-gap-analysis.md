...
- Finding location: src/main/java/com/example/DoctypeShareFolderMapping.java:35:36 (EntityManager.createNativeQuery)
...
- Finding locations:
  - src/main/java/com/example/DoctypeShareFolderMapping.java:35:36 (EntityManager.createNativeQuery)
  - src/main/java/com/example/DoctypeShareFolderMapping.java:55:29 (PanacheEntityBase.list)
...
The generated model pack `.codeql/models/generated-sql-injection-sinks.yaml` produces identical results to the reference model pack `ql/src/quarkus-sinks.model.yml`. Both model packs successfully detect the Panache `list(query)` SQL injection sink at line 55 that is missed by baseline CodeQL analysis. The proof demonstrates that the generated model pack closes the documented Quarkus/Panache framework gap.

Baseline CodeQL (no model pack):
- Detected 1 SQL injection (JPA only)
- Missed the Panache framework sink

Reference model pack (ql/src):
- Detected 2 SQL injections (JPA + Panache)
- Successfully modeled PanacheEntityBase.list as a SQL sink

Generated model pack (.codeql/models):
- Detected 2 SQL injections (JPA + Panache)
- Successfully modeled PanacheEntityBase.list as a SQL sink
- Result matches reference model pack exactly
