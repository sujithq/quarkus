...
- SQL injection not detected in `PanacheEntityBase.list(String query)` call at line 55
- Baseline CodeQL only detected standard JPA methods like `EntityManager.createNativeQuery()`
- Quarkus Panache query methods were not recognized as SQL injection sinks

### After Model Pack

- SQL injection expected to be detected via:
  - `io.quarkus.hibernate.orm.panache.PanacheEntityBase.list(Argument[0])`
- Model pack adds Panache query method as recognized SQL injection sink
- Data flow from `@QueryParam("doctype")` through string concatenation to `list(query)` should now be flagged

### Validation Confidence

- **high**: Clear match between finding and generated sink
  - Finding references `PanacheEntityBase.list(String query)` at line 55
  - Model defines `io.quarkus.hibernate.orm.panache.PanacheEntityBase.list` with `Argument[0]` as SQL injection sink
  - Class and method signatures match exactly
  - Vulnerable parameter position (first argument) correctly modeled
